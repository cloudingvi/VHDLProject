...
In this game, 4 players test their relfex against each other.
...
To play the game again, toggle the first switch off and on again. If less that 4 people are playing, they are allowed to use more than one button to make the game start, and then they can false-start it.

𝜗𝜚 ----------- Displays and LEDs ----------- 𝜗𝜚
The players are assigned numbers, starting from player 1 at the top and going counter-clockwise. A LED around the center will light up for each player that presses their button. The center button is not used.
...
Should any error occurr, the game can be restarted by toggling the last (first on the right) switch twice. Turn the first switch off before resetitng the game. If the FPGA shows IDLE, this has not been done, and the first switch needs to be toggled correctly before playing.
